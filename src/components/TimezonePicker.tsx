...
export default function TimezonePicker({ onClose }: TimezonePickerProps) {
  ...
  const { addTimezone, activeTimezones, recentTimezones } = useTimezoneStore();
        ...
        <div className="relative">
          ...
          >
            <Combobox.Options className="absolute z-10 mt-2 w-full max-h-80 overflow-auto rounded-xl shadow-xl py-1 bg-white dark:bg-gray-700 border-2 border-gray-100 dark:border-gray-600">
              {/* Recent Timezones Section */}
              {query === '' && recentTimezones.length > 0 && (
                <>
                  <div className="px-4 py-2 text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide flex items-center gap-2">
                    <span>🕐</span>
                    Recently Used
                  </div>
                  {recentTimezones.map((tz) => {
                    const tzInfo = getTimezoneInfo(tz);
                    const isAlreadyAdded = activeTimezones.some(active => active.timezone === tz);
                    return (
                      <Combobox.Option
                        key={`recent-${tz}`}
                        value={tz}
                        ...
                        {({ active }) => (
                          <div className="flex justify-between items-center">
                            <div className="flex items-center gap-2">
                              ...
                              <span className="font-medium text-sm text-gray-900 dark:text-white">{tzInfo.displayName}</span>
                              {isAlreadyAdded && <span className="text-xs text-gray-400">(added)</span>}
                            </div>
